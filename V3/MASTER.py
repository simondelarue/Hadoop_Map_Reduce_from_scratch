...
def create_splits(filename, text_window):
    ...
    while file_len > text_window:
        ...
        while (readed_file[split_end] != " " and readed_file[split_end] != '\n'):
            split_end -= 1
...
def deploy_machines(machine):
    ...
    cmd = f'scp {file_to_copy} {getName()}{machine}:{direct}'
...
def main(filename):
    ...
    working_directory = '/tmp/sdelarue'
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '-clean':
        ...
        with open('available_machines.txt', encoding='utf8') as f:
            for line in f.readlines():
    ...
    # Création des shuffles sur chaque worker
    #print('Création des shuffles')
    with Pool() as p:
        log_shuffle = p.starmap(lauch_shuffle, zip(list(FILE_DISTRIBUTION.values()), list(FILE_DISTRIBUTION.keys())))   
    # Envoi des shuffles à travers le réseau
    #print(f'Envoi des shuffles')
    with Pool() as p:
        p.map(network_shuffle, list(FILE_DISTRIBUTION.values()))   
    ...
    print(f'Temps total : {time_MAP + time_SHUFFLE + time_REDUCE:.5f}')
